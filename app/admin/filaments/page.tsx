...
const COMMON_COLOR_NAMES = [
  'Matt Black', 'Silk Silver', 'Silk Red', 'Silk Orange', 'Silk Purple',
  'Silk Blue', 'Silk Green', 'Silk Gold', 'Silk Copper', 'Hyper White',
  'Pure White', 'Cool Gray', 'Warm Gray', 'Anthracite', 'Carbon Black',
  'Metallic Bronze', 'Metallic Gold', 'Neon Yellow', 'Neon Green', 'Neon Pink',
  'Pastel Pink', 'Pastel Blue', 'Pastel Mint', 'Galaxy Black', 'Midnight Blue',
  'Emerald Green', 'Ruby Red', 'Sapphire Blue', 'Titanium Gray', 'Gunmetal',
];

function getStockStatus(grams: number): { label: string; color: string; icon: typeof Check } {
...
export default function FilamentAdminPage() {
  ...
  const resetForm = () => {
    setForm({
      company_id: '', material: 'PLA', color_name: '', color_hex: '#F9733E',
  ...
  const save = async () => {
    if (!form.color_name.trim() || !form.company_id) return;
    ...
    const payload = {
      ...
      grams_remaining: parseInt(form.grams_remaining) || 0,
      grams_total: parseInt(form.grams_total) || 1000,
  ...
  const edit = (f: Filament) => {
    ...
    setForm({
      ...
      cost: String(f.cost),
      grams_remaining: String(f.grams_remaining),
      grams_total: String(f.grams_total),
  ...
  const filtered = filaments.filter(f => {
    ...
    return f.color_name.toLowerCase().includes(q) ||
        ...
        <div className="section-padding">
          <div className="container-wide">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              ...
              >
                <Plus className="w-4 h-4" /> Add Filament
              </button>
            </div>
            ...
            <div className="relative mb-6">
              ...
              <input
                type="text"
                placeholder="Search by color, company, or material..."
                value={search}
                      ...
                      <div className="flex gap-4 p-4">
                        <div className="w-16 h-16 rounded-lg flex-shrink-0 overflow-hidden border border-border" style={{ backgroundColor: f.color_hex }}>
                          {f.image_url && (
                            ...
                            <img src={f.image_url} alt={f.color_name} className="w-full h-full object-cover" />
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="flex gap-1">
                              <button onClick={() => edit(f)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-[#F9733E] transition-colors">
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button onClick={() => remove(f.id)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-red-500 transition-colors">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${stock.color}`}>
            ...
            <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="forge-input appearance-none pr-10"
                    >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {COMMON_COLOR_NAMES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-xs font-semibold tracking-widest text-muted-foreground uppercase mb-2 block">Cost (OMR)</label>
                    <input type="number" step="0.001" placeholder="0.000" value={form.cost} onChange={e => setForm(prev => ({ ...prev, cost: e.target.value }))} className="forge-input" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold tracking-widest text-muted-foreground uppercase mb-2 block">Remaining (g)</label>
                    <input type="number" placeholder="1000" value={form.grams_remaining} onChange={e => setForm(prev => ({ ...prev, grams_remaining: e.target.value }))} className="forge-input" />
                  </div>
                  <div>
                    <label className="text-xs font-semibold tracking-widest text-muted-foreground uppercase mb-2 block">Total (g)</label>
                    <input type="number" placeholder="1000" value={form.grams_total} onChange={e => setForm(prev => ({ ...prev, grams_total: e.target.value }))} className="forge-input" />
                  </div>
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors ${form.is_active ? 'bg-[#F9733E]' : 'bg-muted'}`}
                  >
              ...
              <div className="p-6 border-t border-border flex gap-3 sticky bottom-0 bg-card">
                ...
                <button
                  onClick={save}
                  disabled={!form.color_name || !form.company_id}
