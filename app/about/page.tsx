    ...
    <>
      ...
      <main className="pt-16 bg-background pb-16 md:pb-0">
        ...
        <section className="section-padding bg-muted/30 text-foreground relative overflow-hidden">
          <div className="absolute top-1/4 -left-32 w-96 h-96 rounded-full bg-[#F9733E]/8 blur-3xl" />
          ...
          <div className="container-wide relative">
            <ScrollReveal className="max-w-3xl">
              ...
              <h1 className="text-fluid-hero font-display leading-[0.95]">Two brothers.<br />A printer.<br /><span className="text-gradient">A desire to make things personal.</span></h1>
              <p className="text-fluid-body text-muted-foreground max-w-xl mt-6 leading-relaxed">Twin Forge Co. started at a kitchen table in Oman — two brothers, Marwan and Muhannad, a 3D printer, and a frustration with one-size-fits-all gear. We design components for gamers, tinkerers, and anyone who wants something built their way.</p>
            </ScrollReveal>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <ScrollReveal>
                <div className="relative">
                  <div className="aspect-[4/5] rounded-2xl overflow-hidden border border-border img-zoom">
                    <img src="https://images.pexels.com/photos/5089174/pexels-photo-5089174.jpeg?auto=compress&cs=tinysrgb&h=650&w=940" alt="A craftsman working in a workshop" className="w-full h-full object-cover" loading="lazy" />
                  </div>
                  <div className="absolute -bottom-4 -right-4 bg-[#F9733E] text-white px-5 py-4 rounded-xl shadow-forge-orange">
                    <p className="font-display text-3xl font-bold leading-none">2</p><p className="text-xs mt-1">brothers</p>
                  </div>
                </div>
              </ScrollReveal>
              <ScrollReveal delay={150}>
                <div>
                  ...
                  <h2 className="text-fluid-display font-display text-foreground mb-6">To let users design products that match their hands, playstyles, and tastes.</h2>
                  <div className="space-y-4 text-muted-foreground leading-relaxed">
                    <p>We come from engineering and making — Marwan lives in CAD, Muhannad lives on the printer. Between us we&apos;ve broken, rebuilt, and refined hundreds of prototypes to get the weight, balance, and feel right.</p>
                    <p>The plastic butterfly trainer was our first real product because it sits at the intersection of everything we love: mechanical precision, tactile feel, and the satisfaction of holding something you designed yourself. Made from safe PLA plastic, it&apos;s suitable for flippers of all ages.</p>
                    <p>Everything we ship is made to order in small batches in Oman. No warehouses of inventory. No compromise on the finish. Just the forge, the two of us, and whatever you dream up in the configurator.</p>
                  </div>
                </div>
              </ScrollReveal>
            </div>
              ...
              {VALUES.map((value, i) => (
                <ScrollReveal key={value.title} delay={i * 120}>
                  <div className="h-full p-8 rounded-2xl bg-card border border-border hover:border-[#F9733E]/30 transition-all duration-500 hover:-translate-y-1">
                    <div className="p-3 rounded-lg bg-[#F9733E]/10 w-fit mb-5"><value.icon className="w-6 h-6 text-[#F9733E]" /></div>
              ...
              {[
                ...
                <ScrollReveal key={step.label} delay={i * 80}>
                  <div className="relative group">
                    <div className="absolute -top-3 -left-3 w-7 h-7 rounded-full bg-[#F9733E]/10 border border-[#F9733E]/30 flex items-center justify-center text-[#F9733E] text-[10px] font-bold z-10">{i + 1}</div>
                    <div className="rounded-lg overflow-hidden border border-border bg-card hover:border-[#F9733E]/40 transition-all duration-500 h-full">
                      <div className="h-20 img-zoom"><img src={step.img} alt={step.label} className="w-full h-full object-cover" loading="lazy" /></div>
                      <div className="p-4"><step.icon className="w-5 h-5 text-[#F9733E] mb-2" /><p className="font-heading font-semibold text-sm">{step.label}</p><p className="text-[10px] text-muted-foreground mt-0.5">{step.desc}</p></div>
                    </div>
                  </div>
                </ScrollReveal>
              ))}
            ...
            <ScrollReveal>
              <div className="relative rounded-3xl bg-gradient-to-br from-[#F9733E] to-[#e85e28] text-white p-12 md:p-16 overflow-hidden">
                ...
                <div className="relative max-w-xl">
                  <h2 className="text-fluid-display font-display mb-4">Build something that&apos;s yours.</h2>
                  <p className="text-fluid-body text-white/90 mb-8 max-w-md">Every product is made to order. Browse the shop and spec one to your exact taste — color, weight, finish.</p>
                  <Link href="/shop" className="inline-flex items-center gap-2 px-7 py-4 bg-white text-[#F9733E] font-semibold rounded-lg hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200">Browse the Shop<ArrowRight className="w-4 h-4" /></Link>
                </div>
              </div>
            </ScrollReveal>
          </div>
