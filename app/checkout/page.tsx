...
export default function CheckoutPage() {
  ...
  const packingFee = items.some(i => i.id.includes('butterfly') || i.id.includes('trainer') || i.id.includes('configurator')) ? 1 : 0;
  const grandTotal = totalPrice + packingFee;
  ...
  const validate = () => {
    ...
    else if (!/^[0-9]{8}$/.test(form.phone.trim())) e.phone = 'Phone must be exactly 8 digits';
      ...
      <>
        ...
        <main className="pt-16 min-h-screen bg-background flex items-center justify-center px-6 pb-16 md:pb-0">
          <div className="max-w-md text-center">
            ...
            <p className="text-xs text-muted-foreground mb-6">Order ID: <span className="font-mono font-semibold text-foreground">ORD-{String(orderId).padStart(4, '0')}</span></p>
            <p className="text-sm text-muted-foreground mb-8">We&apos;ll contact you on <span className="font-semibold text-foreground">{COUNTRY_PREFIX} {form.phone}</span> to confirm delivery details.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" value={form.name} onChange={e => setForm(prev => ({ ...prev, name: e.target.value }))} className="w-full pl-10 pr-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200 placeholder:text-muted-foreground" placeholder="Your name" />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex">
                        <div className="flex items-center px-3 py-3 bg-muted border border-r-0 border-border rounded-l-lg text-sm text-muted-foreground font-semibold select-none">{COUNTRY_PREFIX}</div>
                        <div className="relative flex-1">
                          <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none z-10" />
                          <input type="tel" value={form.phone} onChange={e => setForm(prev => ({ ...prev, phone: e.target.value.replace(/[^0-9]/g, '') }))} className="w-full pl-10 pr-4 py-3 bg-white border border-border rounded-r-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200 placeholder:text-muted-foreground" placeholder="12345678" maxLength={8} />
                        </div>
                      </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="relative">
                        ...
                        <input type="email" value={form.email} onChange={e => setForm(prev => ({ ...prev, email: e.target.value }))} className="w-full pl-10 pr-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200 placeholder:text-muted-foreground" placeholder="you@email.com" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select value={form.governorate} onChange={e => setForm(prev => ({ ...prev, governorate: e.target.value, city: '' }))} className="w-full px-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200">
                        {GOVERNORATES.map(g => <option key={g} value={g}>{g}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground uppercase mb-2 block">City / Area *</label>
                      <select value={form.city} onChange={e => setForm(prev => ({ ...prev, city: e.target.value }))} className="w-full px-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200">
                    ...
                    <div>
                      ...
                      <textarea value={form.address} onChange={e => setForm(prev => ({ ...prev, address: e.target.value }))} className="w-full px-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200 placeholder:text-muted-foreground min-h-[80px] resize-y" placeholder="Building number, street name, nearest landmark..." />
                ...
                <div className="rounded-2xl border border-border bg-card p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button onClick={() => setForm(prev => ({ ...prev, shippingMethod: 'home_delivery' }))} className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all ${form.shippingMethod === 'home_delivery' ? 'border-[#F9733E] bg-[#F9733E]/5' : 'border-border hover:border-foreground/30'}`}>
                      ...
                      <div><p className="text-sm font-semibold text-foreground">Home delivery</p><p className="text-xs text-muted-foreground mt-0.5">Delivered to your door · 2 OMR</p></div>
                    </button>
                    <button onClick={() => setForm(prev => ({ ...prev, shippingMethod: 'office_pickup' }))} className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all ${form.shippingMethod === 'office_pickup' ? 'border-[#F9733E] bg-[#F9733E]/5' : 'border-border hover:border-foreground/30'}`}>
                      ...
                      <div><p className="text-sm font-semibold text-foreground">Office pickup</p><p className="text-xs text-muted-foreground mt-0.5">Nearest {SHIPPING_AGENT} office · 1 OMR</p></div>
                    </button>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-6">
                  ...
                  <textarea value={form.notes} onChange={e => setForm(prev => ({ ...prev, notes: e.target.value }))} className="w-full px-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F9733E] focus:border-transparent transition-all duration-200 placeholder:text-muted-foreground min-h-[60px] resize-y" placeholder="Any special instructions..." />
                </div>
              </div>
              ...
              <div className="lg:sticky lg:top-24 self-start">
                <div className="rounded-2xl border border-border bg-card overflow-hidden">
                  ...
                  <div className="p-6 border-t border-border space-y-2">
                    ...
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Shipping ({SHIPPING_AGENT})</span><span className="font-semibold text-amber-600">{fmt(shippingCost)} · Paid on delivery</span></div>
                    {packingFee > 0 && (<div className="flex justify-between text-sm"><span className="text-muted-foreground">Packing fee</span><span className="font-semibold text-foreground">{fmt(packingFee)}</span></div>)}
                    ...
                    <p className="text-[11px] text-muted-foreground pt-1">+ {fmt(shippingCost)} shipping fee paid to {SHIPPING_AGENT} on delivery</p>
                  </div>
